...
int main(int argc, const char *argv[])
{
    ...
    //int imgEndIndex = 5;   // last file index to load
    int imgStepWidth = 1; 
    ...
    for (int p = 0; p < DetectorTypeVector.size(); p++)
    {
        ...
        for (int q = 0 ; q < DescriptorTypeVector.size()  ; q++) // akaze는 별도로 
        {
            ...
            string filenameDetector = DetectorTypeVector[DetectorTypeVectorCount];
            string filenameDescriptor = DescriptorTypeVector[DescriptorTypeVectorCount];
            string csvExtension = ".csv";

        
            std::ofstream myfile;
                
            myfile.open (filenameDetector+filenameDescriptor+csvExtension);
            myfile << "Detector: " << DetectorTypeVector[DetectorTypeVectorCount] <<" "<<"Descriptor: " << DescriptorTypeVector[DescriptorTypeVectorCount] << "\n";
            ...
            for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
            {
                ...
                if(bVis)
                {
                    show3DObjects((dataBuffer.end()-1)->boundingBoxes, cv::Size(4.0, 20.0), cv::Size(1000, 1000), bVis);
                }
                //bVis = false;
                ...
                string detectorType = DetectorTypeVector[DetectorTypeVectorCount];
                //vector<string> DetectorTypeVector = {"SHITOMASI", "HARRIS", "FAST", "BRISK", "ORB", "AKAZE", "SIFT"};
                ...
                cv::Mat descriptors;
                string descriptorType = DescriptorTypeVector[DescriptorTypeVectorCount]; // BRISK, BRIEF, ORB, FREAK, AKAZE, SIFT
